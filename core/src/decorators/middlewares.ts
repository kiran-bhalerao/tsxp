...
/**📢 Comma separated middlewares, Ex. @Middlewares( logger, handler, .. ) */
  ...
  return (_target: U, _key: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async function (req: Request, res: Response, next: NextFunction) {
      ...
      handlers.forEach((fn, i) => {
        if (handlers[i + 1]) {
          table[i] = () => fn(req, res, async () => await throwable(table[i + 1]))
        } else
          table[i] = () =>
            fn(req, res, async () => await throwable(async () => await originalMethod.apply(this, [req, res, next])))
      })
