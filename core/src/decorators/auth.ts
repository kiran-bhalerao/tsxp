...
const defaultAuthChecker = (_: Request, acceptedRole: string[], currentRole?: string) => {
  if (!currentRole) {
    return false
  }
...
export function Auth<T extends string>(props?: AuthType<T>) {
  return (_target: U, _key: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async function (req: Request, res: Response, next: NextFunction) {
      ...
      const error = props?.extends?.props?.error || props?.error || 'Invalid Permissions'
      const userTypeResolver = props?.extends?.userTypeResolver || defaultUserTypeResolver
      ...
      await throwable(async () => await originalMethod.apply(this, [req, res, next]))
    }
...
abstract class AuthExtender<T extends string> {
  ...
  public authChecker(req: Request, acceptedRole: string[], currentRole?: string) {
